...
import sys
include = ['src', *sys.path, 'tests', '/mingw/bin', '/mingw/include']
cd = {'language_level' : "3"}
...
args = ['-O2', '-funsafe-math-optimizations', '-ffast-math']
...
setup(
    ext_modules=cythonize([Extension("tests.cynum", sources=["tests/cynum.pyx"], include_dirs=include, extra_compile_args=args, extra_link_args=link_args, define_macros=[('CYTHON_TRACE_NOGIL', '1'), ('CYTHON_TRACE', '1')])], nthreads=12, show_all_warnings=1, annotate=1),
    zip_safe=False, include_dirs=include
)
