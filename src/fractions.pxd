...
static double inline c_n_pow(double n1, const double pow) {
    ...
    static _frac_ptr inline _frac_2_fracptr(const struct _fraction frac){
        return &frac;
    }
    '''
    cdef struct _fraction:
        ...
        double denominator
    ctypedef (_fraction*) _frac_ptr
    const _fraction _fracptr_2_frac(const _frac_ptr ptr) noexcept nogil
    const _frac_ptr _frac_2_fracptr(const _fraction ptr) noexcept nogil
...
cdef inline void _set_num_den(_fraction* first, const double numerator, const double denominator) noexcept nogil:
...
cdef inline _Bool _eq_fractions(const _fraction* first, const _fraction* second, const _Bool div) noexcept nogil:
    cdef _Bool b
    if div:
...
cdef inline _Bool _eq_fraction_double(const _fraction* first, const double second) noexcept nogil:
...
cdef inline _Bool _less_than_double(const _fraction* first, const double second) noexcept nogil:
    return (first.numerator) < (second*first.denominator)

cdef inline _Bool _greater_than_double(const _fraction* first, const double second) noexcept nogil:
    return (first.numerator) > (second*first.denominator)
...
cdef inline _Bool _greater_than(const _fraction* first, const _fraction *second) noexcept nogil:
    return (first.numerator*(second).denominator) > ((second).numerator*first.denominator)

cdef inline _Bool _less_than(const _fraction* first, const _fraction *second) noexcept nogil:
    return (first.numerator*(second).denominator) < ((second).numerator*first.denominator)

cdef inline void _simplify_fraction(_fraction* first) noexcept nogil:
...
cdef inline void _fix_num_den(_fraction* first) noexcept nogil:
    if (first.denominator < 0):
        first.denominator = -first.denominator
        first.numerator = -first.numerator

cdef inline _fraction _mult_fractions(_fraction first, const _fraction* second) noexcept nogil:
    first.numerator = first.numerator*second.numerator
    first.denominator = first.denominator*second.denominator
    return first

cdef inline _fraction _div_fractions(_fraction first, const _fraction* second) noexcept nogil:
    first.numerator = first.numerator*second.denominator
    first.denominator = first.denominator*second.numerator
    _fix_num_den(&first)
    return first

cdef inline _fraction _add_fractions(_fraction first, const _fraction* second) noexcept nogil: # first is the greater
...
cdef inline double _as_double(const _fraction* first) noexcept nogil:
    return first.numerator/first.denominator

cpdef inline _Bool greater_than(const _fraction first, const _fraction second) noexcept nogil:
    return _greater_than(&first, &second)

cpdef inline _Bool less_than(const _fraction first, const _fraction second) noexcept nogil:
...
cpdef inline _Bool eq_fractions(const _fraction first, const _fraction second, const _Bool div) noexcept nogil:
    return _eq_fractions(&first, &second, div)
